...
In the browser or desktop, open **Compare** and use the visible inspection workflow: select a named interface, inspect its sections, verify when bounded evidence is needed, then save or capture. Create and edit interfaces under **Advanced: edit interfaces and section frames**. Each feature can hold several independently oriented section series; select, add and remove them in the editor. Enter a plane origin, normal and horizontal direction in the aligned part frame, or frame a curved interface in the 3D view and choose **Use current view as frame** before adding station offsets. Offsets move parallel planes along that series' normalized normal. Zero contour simplification preserves small shoulders and lips. **Inspect preview** overlays CAD in amber and the reference in blue in one local millimetre frame. Cuts are restricted to the named region without artificial caps. Open contours stay open; this view does not invent material or certify a mating fit.
...
The preview section view uses the cached display mesh. It can omit small details that are present in the CAD. **Verify precisely** runs the saved feature series through the wall-clock, child-memory, triangle and cancellation limits in the collapsed verification settings. A completed request displays its matching verified contours automatically; **Show verified sections** remains available when returning to an earlier result. JSON/SVG exports retain the verification request and meshing provenance. **Record this inspection** preserves that selected source, request and provenance in the card; it never recomputes a preview when recording verified contours. Cancellation or a newer request expires the displayed verified result. A verified failure remains unavailable and never substitutes preview sections. Requested deflection is not an achieved error bound. Review each important local feature and retain uncertainty or fit-coupon evidence alongside sampled distances.
...
A model-specific exact validator can publish its verdict beside these local inspections without turning a Markdown link into an acceptance claim. Declare `validation_reports = [{ file = "references/current-acceptance.json", label = "Interface validation" }]` on the target. The report must contain a `viewer_evidence` contract made with `nurb.validator_evidence.viewer_contract()`. That contract names the feature IDs covered by the report and binds the verdict to hashes of portable project inputs, with optional runtime and nurb source identities. The server reads the report without executing it, re-reads every bound input to reject replacements during the snapshot, and watches the report and inputs for later changes. The viewer then distinguishes a current accepted verdict, a current failed verdict, and a stale or unavailable report. The specialized B-rep validator remains authoritative; this surface only carries its compact current verdict and findings.
...
In the primary inspection workflow, inspect the saved feature or run precise verification, then choose **Export section JSON** or **Export station SVG** in the Save or capture step. The embedded desktop viewer saves through the shared artifact bridge and exposes the resulting file. The browser starts a download. Preview exports are explicitly labeled **Display mesh estimate** with unknown accuracy in millimetres. Verified exports include the bounded verification provenance and never fall back to the preview mesh. A changed model, reference, alignment, feature scale or unsaved editor change prevents an old result from being exported. The server also rejects changed source or feature settings before the watcher has rebuilt them.
