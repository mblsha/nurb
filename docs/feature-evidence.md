...
In the browser or desktop, open **Compare → Named inspection regions → Functional feature and local sections**. Select or create a region, enable feature tracking and save it. Each feature can hold several independently oriented section series; select, add and remove them in the editor. Enter a plane origin, normal and horizontal direction in the aligned part frame, or frame a curved interface in the 3D view and choose **Use current view as frame** before adding station offsets. Offsets move parallel planes along that series' normalized normal. Zero contour simplification preserves small shoulders and lips. **Preview saved feature sections** overlays CAD in amber and the reference in blue in one local millimetre frame. Cuts are restricted to the named region without artificial caps. Open contours stay open; this view does not invent material or certify a mating fit.
...
The preview section view uses the cached display mesh. It can omit small details that are present in the CAD. **Verify with a finer CAD mesh** runs the saved feature series through the same wall-clock, child-memory, triangle and cancellation limits as the precise comparison. After it completes, **Show verified sections** displays those contours and JSON/SVG exports retain the verification request and meshing provenance. **Record this inspection** preserves that selected source, request and provenance in the card; it never recomputes a preview when recording verified contours. Cancellation or a newer request expires the displayed verified result. Requested deflection is not an achieved error bound. Review each important local feature and retain uncertainty or fit-coupon evidence alongside sampled distances.
...
In **Functional feature and local sections**, preview the saved feature or run precise verification and choose **Show verified sections**, then choose **Export section JSON** or **Export station SVG**. The embedded desktop viewer saves through the shared artifact bridge and exposes the resulting file. The browser starts a download. Preview exports are explicitly labeled **Display mesh estimate** with unknown accuracy in millimetres. Verified exports include the bounded verification provenance and never fall back to the preview mesh. A changed model, reference, alignment, feature scale or unsaved editor change prevents an old result from being exported. The server also rejects changed source or feature settings before the watcher has rebuilt them.
